...
set s=(select name from class where id=1);
return s;
end $$
delimiter ;
...
#  3. 1编写一个函数，传入两个学生的ID，返回
     这两个学生成绩之差
delimiter $$
create function subtract(sid1 int,sid2 int)
returns int
begin
declare a int;
declare b int;
select score from class where id=sid1 into a
select score from class where id=sid2 into b
return a-b
end $$
delimiter ;
...
delimiter $$
create procedure get_score4(INOUT num varchar(32))
begin
set num=(select score from class where name=num);
end $$
delimiter ;
set @num="张飞";
call get_score4(@num);
...
select name from mysql.proc where db='stu' and type='procedure' or db='stu' and type="function";
select name from mysql.proc where db='stu' and (type='procedure' or type="function");

DROP  FUNCTION IF EXISTS get_score4;
