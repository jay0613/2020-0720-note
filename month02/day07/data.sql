...
1. 查询每位老师教授的课程数量
select tname,count(teacher_id) from teacher left join course on course.teacher_id=teacher.tid group by tname;
2. 查询学生的信息及学生所在班级信息
select caption,sname,gender from class inner join student on class.cid=student.class_id;
3. 查询各科成绩最高和最低的分数,形式 : 课程ID  最高分  最低分
select cid,cname,max(number),min(number) from score right join course on course.cid=score.course_id group by cid,cname;
4. 查询平均成绩大于85分的所有学生的学号,姓名和平均成绩
select
