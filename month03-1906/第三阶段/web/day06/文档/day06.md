...
##### 2. switch语句(02-switch.html)

+ 语法 :
```javascript
switch(value){
	 case 值1 :
	 //value与值1匹配全等时,执行:后面的代码段
	 break; //结束匹配
	 case 值2 :
	 //value与值2匹配全等时,执行:后面的代码段
	 break;
	 case 值3 :
     //value与值3匹配全等时,执行:后面的代码段
	 break;
	 default:
 	 //所有case匹配失败后默认执行:后面的语句
 	 break;
}
```
+ 使用 :
