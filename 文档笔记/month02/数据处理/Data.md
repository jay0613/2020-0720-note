...
* 字节串类型
  ...
  * 字符串与字节串相互转化方法
    ...
    ```python
    - 普通的英文字符字符串常量可以在前面加b转换为字节串，例如：b'hello'，汉字等等字符串必须使用下面的方法转换字符串，不能直接加b转换
  ...
  * 并不是所有的字节串都能转换为字符串
  ...
  print(b3)  # b'hello ,\xe6\xad\xbb\xe9\xac\xbc'
  # 字节串转换为字符串
  print("转换为字符串：",b'hello ,\xe6\xad\xbb\xe9\xac\xbc'.decode())  # 转换为字符串： hello ,死鬼
  ```
...
| a        | 以追加写模式打开，文件不存在则创建，存在则继续进行写操作     |
| r+       | 以读写模式打开 文件必须存在       # 写入的时候会从前往后覆盖内容 |
...
返回值： 返回读取到的内容
f = open("file.txt", "r")
data = f.read(10)  #一次读取10个字符，下次读取会接着上次的继续读取  # 空格不是空字符，直到真正读取到最后才会返回空字符。
```
...
返回值： 返回读取到的内容
f = open("file.txt", "r")
data = f.readline()  # #一次读取1行字符，包括行末的换行符，下次读取会接着上次的继续读取  # 空格不是空字符，直到真正读取到最后才会返回空字符。
```
...
返回值：写入的字符个数
f = open("file.txt","w")
n = f.write("你好啊")  # 把“你好啊”写入file.txt里面，并返回写入的字符个数
print("写入了%d个字符"%n)

f = open("file.txt","wb")
n = f.write(b"hello")  # 把“你好啊”写入file.txt里面，并返回写入的字符个数
print("写入了%d个字节"%n)
```
...
f = open("file.txt","w")
list01 = ["哈喽","你好"]
f.writelines(list01)  # 哈喽和你好在同一行
```

```python
"""
空洞文件   例如迅雷下载电影，点击下载就会立刻先占用空间再下载
"""
f = open("file.txt",'wb')
f.write(b"begin")
f.seek(100000,2)
...
"""
# 打开源文件
fr = open("/home/tarena/timg.jfif",'rb')  # 打开复制的文件
fw = open("timg.jfif",'wb')  # 复制到该文件内
# 循环读写内容
while True:
    # 一边读取，一边写入
    data = fr.read(1024)
...
while True:
    ...
    if not info:
        break
    f.write(info.encode())  # 写入字符超过10字节则立即写入文件(上面设置了缓冲区大小),没超过10字节则当程序终止或者关闭时候才会写入文件
    # f.flush() # 主动刷新缓冲区,立刻把缓冲区文件写入
f.close()
```
...
* 定义
  ...
  > 2. a方式打开，文件偏移量在文件结尾位置,但是a+方式打开时候要注意偏移量，最好用tell（）查看一下偏移量
...
os模块是Python标准库模块，包含了大量的文件处理函数。,需要import导入
...
> 格式：`(?P<name>pattern)`    # P 是大写    一般用re.search（）使用
...
In [28]: re.search(r'(?P<tel>1\d{10})\D',"13713788072是
    ...: 我的电话").group("tel")
Out[28]: '13713788072'
```
...
#### 2.4.2  生成match对象（以下方法查找到的结果得 .group()再打印才能看到结果）
...
功能：匹配某个目标字符串开始位置,只有目标字符串在开头位置才能匹配到
参数：pattern 正则
	string  目标字符串
...
- span()  获取匹配内容的起止位置  # 例如（3,12）   是一个切片
...
* 常用flag
  ...
  ```python
  A == ASCII  元字符只能匹配ascii码   #flags=re.A  # 用了这个之后，使用\w之后不能匹配中文了  下面有举例
  
  I == IGNORECASE  匹配忽略字母大小写  # flags=re.I
  
  S == DOTALL  使 . 可以匹配换行    # flags=re.S
  
  M == MULTILINE  使 ^  $都可以匹配每一行的开头结尾位置 # flags=re.M   # 
...
```sql
e.g. 创建stu数据库，编码为utf8    mysql  8.0版本以上默认编码为utf8
...
* 数字类型：
  ...
  * 比特值类型：BIT      # 只能存0 或者   1
  * int(2)   # 只是显示的宽度为2，可插入的数据范围还是-2147483648，2147483647

![](./img/整型.png)

> 注意：
>
> 1. 对于准确性要求比较高的东西，比如money，用decimal类型减少存储误差。声明语法是DECIMAL(M,D)。M是数字的最大数字位数（包括小数的位数），D是小数点右侧数字的位数。比如 DECIMAL(6,2)最多存6位数字，小数点后占2位,取值范围-9999.99到9999.99。
> 2. 比特值类型指0，1值表达2种情况，如真，假   # bit
...
> 注意：
>
> 1. char：定长，即指定存储字节数后，无论实际存储了多少字节数据，最终都占指定的字节大小。默认只能存1字节数据。存取效率高。  # char(20)   指定20个字节 ,但是例如只储存10字节，也一样是占用20字节   #char参数最多255
> 2. varchar：不定长，效率偏低 ，但是节省空间，实际占用空间根据实际存储数据大小而定。必须要指定存储大小 varchar(50)  ，指定最多50字节，但是例如只储存10字节，那就只占用10字节
...
* 字段约束
  * 如果你想设置数字为无符号则加上 unsigned      # 数字类型设置无符号之后，没有负数，正数的范围翻倍。
  * 如果你不想字段为 NULL 可以设置字段的属性为 NOT   NULL（不输入就会报错）， 在操作数据库时如果输入该字段的数据为NULL ，就会报错。
  * DEFAULT 表示设置一个字段的默认值  # 例如设置默认值为0  ----->>>   DEFAULT  0       # TEXT类型不能设置默认值
  * AUTO_INCREMENT定义列为自增的属性，一般用于主键，数值会自动加1。# 先写下面一行再写这行 #auto_increment
...
```sql
insert 库A.表A(字段1,字段2,字段3,字段4,字段5) select 字段1,字段2,字段3,字段4,字段5 from 库B.表B;
例如：
insert books.book(title,author,publication,price,comment) select 书名,作者,出版社,价格,备注 from stu.book;  # 其中一个表我用了中文字段名
```
...
* 时间格式
  ...
  >
  > 1. datetime ：以系统时间存储
  > 2. timestamp ：以标准时间存储但是查看时转换为系统时区，所以表现形式是和datetime相同，如果不给该字段赋值，该字段返回系统当前时间
...
* 模糊查询和正则查询
  ...
  1. 模糊查询
     ...
     LIKE用于在where子句中进行模糊查询，SQL LIKE 子句中使用
...
```sql
	...
	# 可以查找出来Ale、Asfs、agfh等等A开头的名字,不区分大小写
# LIKE类似平时的“==”号
```
...
# 用.匹配汉字的时候1个汉字代表2个或者3个字符，utf8的时候是3个字符
```
~~~
* as 用法
...
* 排序
  ...
  使用 ORDER BY 子句将查询数据排序后再返回数据：
	...
	```sql
	select * from class_1 where sex='m' order by age desc; # 降序
	select * from class_1 where sex='m' order by age;  # 升序
	```
```sql
...
复合排序：对多个字段排序，即当第一排序项相同时按照第二排序项排序
	...
	```sql
	select * from class_1 order by score desc,age desc;    
```
...
* 限制
  ...
  LIMIT [num]
  例如：
  select * from class limit 3;   # 只显示3条信息
  update class set score=90 limit 2;  # 只改前面两条信息
  select * from class order by score desc limit 1;    # 显示成绩最高分的一位
  ```
...
*  联合查询
	...
	默认UNION后为DISTINCT表示删除结果集中重复的数据。如果使用ALL则返回所有结果集，	包含重复数据。

```sql
select * from class_1 where sex='m' UNION ALL select * from class_1 where age > 9;   # 两次查询到的重复数据也会显示

# 两次查询到的重复数据 只显示一条
select * from class_1 where sex='m' UNION select * from class_1 where age > 9;
 
 # order by id  总的查询结果根据id
select * from class where sex='m' UNION select * from class where age > 9 order by id;
```
* 子查询
	...
	* 子查询出现的位置：
		...
		```sql
		# 必须得写as语句  # 在性别为m的查询结果里面查询score>90的;
		select name from (select * from class_1 where sex='m') as s where s.score > 90;  
		
		select m.name,m.score from (select * from class where sex='m') as m where m.score > 80;	
		```
	...
	>注意：  需要将子查询结果集重命名一下，方便where子句中的引用操作
		...
		```sql
		 	...
		 	# 在class里查询分数比关羽高分的人
		 	select * from class where score > (select score from class where name="关羽");
		```
		> 注意：
		>
...
e.g. : 所有国家的男英雄中 英雄数量最多的前2名的   显示国家名称及英雄数量       # 男英雄最多的两个国家
...
> 2. having语句存在弥补了where关键字不能与聚合函数联合使用的不足,where只能操作表中实际存在的字段。# 例如上面的代码 having位置不能写where
> 3. having和where 一样的功能 只不过having后面可以放聚合函数，而where
...
**把一个字段设置主键之后，通过该字段查找数据会比之前速度快几百倍，但是会占用储存空间，(一般需要查找数据超过1亿以上才使用)**

- **优缺点**
  - 优点 ： 加快数据检索速度,提高查找效率(一般需要查找数据超过1亿以上才使用)

  - 缺点 ：占用数据库物理存储空间，当对表中数据更新时(增删改),索引需要动态维护,降低数据写入效率
...
create index name_index on class(name);   # 创建name的索引，索引名为name_index
```
...
create unique index name_unique on class(name); # 创建name的唯一索引，索引名name_unique，注意name字段里面的姓名不能有重复 要不创建不了
```
...
show profiles  查看语句执行信息  # 可以看到最近执行的查询语句的详细用时，精确到小数点后8位
```
...
* 约束 : 约束是一种限制，它通过对表的行或列的数据做出限制，来确保表的数据的完整性、唯一性，创建外键的时候自动创建索引。
* foreign key 功能 : 建立表与表之间的某种约束的关系，由于这种关系的存在，能够让表与表之间的数据，更加的完整，关连性更强，为了具体说明创建如下部门表和人员表。
* “从”表的外键的数据类型必须和绑定的“主”表主键的数据类型一致
* 示例
...
- foreign key 外键的定义语法：    # 建立外键关联之后有三种情况，请参考下面的  级联动作；

  ```sql
  #  symbol 是自己起的名   #constraint symbol 这两个词可以不写，功能就是创建外键及索引名称，不写的话就默认自动创建外键名称，索引名称就是该字段名称；查看外键名称方法：show create table person;
  # 语法：
  constraint symbol foreign key（需要和主表绑定的字段） references 主表(主表主键)   
...
```
  ...
  该语法可以在 CREATE TABLE 和 ALTER TABLE 时使用
	...
	CREATE TABLE person (
	  ...
	  hire_date date NOT NULL,
	  dept_id int ,
	  constraint dept_fk foreign key(dept_id) references dept(id));  # 关键是这一行，设置外键，把dept_id绑定dept(id)  # dept_fk是自己起的名，多次起的名不能重名   # constraint dept_fk 这两个词可以不写，创建外键名称，不写的话就默认自动创建外键名称
```

	```sql
	# 建立表后增加外键
	alter table person add constraint dept_fk foreign key(dept_id) references dept(id);  # dept_fk是自己起的外键名
```
...
* 通过外键名称解除外键约束     # 一般删除外键之后还会删除索引

	```sql
	alter table person drop foreign key dept_fk;    # 删除外键
...
* 级联动作
  ...
  * cascade ：数据级联更新  on delete cascade   on update cascade
    ...
    * 当主表删除记录或更改被参照字段的值时,从表会级联更新（从表也会删除或者更改）
  ...
  * set null  :   on delete set null    on update set null
    ...
    * 当主表更改主键字段值时，从表外键字段值变为null
      ...
      alter table person add 
      constraint dept_fk foreign key(dept_id) 
      references dept(id) on delete set null on update set null; 
      ```
...
>
> “#  在设置的外键后面加unique  代表唯一索引不能重复，而另一张表的主键也是不能重复的，所以是一对一的关系
>
...
>
> "#  可以另外再创建个“从”表来关联这两个“主”表
>
> 举例：一个运动员可以报多个项目，每个项目也会有多个运动员参加,这时为了表达多对多关系需要单独创建关系表。又如：需要添加运动员的成绩，一般添加在从表内
...
ON 表1.字段 = 表2.字段;         # 表1为左表也就是上图的A  表2为右表也就是上图的B
```
...
> 注意：我们尽量使用数据量大的表作为基准表，即左表，提取速度会快一点
...
```sql
语法结构：
# OR REPLACE 加了这个属性之后创建视图的时候如果已经有同名视图，不会报错，会覆盖原来视图。不加这个属性，遇到同名视图，创建时候会报错。
...
e.g.
# c1为起的视图名字,以表的形式存在库中  # select * from c1；可以查到c1表里的内容
例如：
...
* 查看现有视图
  ...
  ```sql
  show full tables in stu where table_type like 'VIEW';    # stu为当前使用的库名
  ```
...
* 删除视图
  ...
  ```sql
  drop view if exists c1;   # c1为视图名
  ```

* 修改视图   # 增加了score字段

  参考创建视图，将create关键字改为alter   # create为创建视图时候的语法
...
“ # 有返回值得函数叫函数，没有返回值的函数叫储存过程

“ # 注意函数创建的名字不能重名，与其他的库的函数名也不能重名
...
```sql
e.g. 无参数的函数调用
delimiter $$      # 把sql语句的结尾符从；改成$$，后面的sql语句结尾符都用$$
...
* 设置变量
  * 用户变量方法：   set  @[变量名] = 值；使用时用：把@[变量名]传进去就行。 # 类似全局变量
  
  * 局部变量 ： 在函数内部设置  declare [变量名] [变量类型] ；局部变量可以使用set赋值或者使用into关键字。
  
    ```sql
    delimiter $$
    create function get_name2()
    returns varchar(30)
    begin
    declare s varchar(30);  # 设置变量名为 s  注意该变量名不要和查询的字段名重名
    set s=(select name from class where id=1); # 把结果赋值给s
    return s;
    end $$
    delimiter ;
    ```
...
e.g. 存储过程创建和调用
delimiter $$
create procedure st()    # 起的存储过程的名字为st
begin 
    select name,age from class_1; 
...
语法：     # 显示内容：数据库、名字、类型、创建者、创建和修改日期

```mysql
show {procedure|function} status [like’存储过程或存储函数的名称’]
#例如
show function status like “st”;  # 查看st函数的创建信息
```
...
# 例如
show create  function  subtract;  查看subtract函数的定义(可以看到创建时候的代码)
```
...
5. 查看所有函数或者存储过程
   ...
   # 查看stu库里的所有函数和储存过程
   select name from mysql.proc where db='stu' and (type='procedure' or type="function");
   ```
...
# 例如   # IF EXISTS写了之后，删除的时候如果没有这个函数也不会报错
DROP  PROCEDURE IF EXISTS get_score4;  # 删除储存过程get_score4
DROP  FUNCTION IF EXISTS subtract;   # 删除函数subtract
```
...
DML语句 :  select    delete    update    insert

DCL语句  :  commit   rollback    grant   revoke

**注意：rollback回滚无法恢复DDL语句操作的数据**
...
```mysql
   mysql>begin; # 方法1
```
2. 开始执行事务中的若干条SQL命令（增删改）# 只能操作表内数据，如果删除了库等等不属于事务内容
...
* 隔离级别
  ...
  >
     >  例如事务A和事务B都开启了事务，事务A修改了一条数据但未提交 ，B那边就已经可以看到A删除后的结果了
   ...
   - 读已提交：read committed
     ...
     > 这种隔离级别会导致“不可重复读取”
     >
     > 例如：事务A和事务B都同时begin开始了事务，事务A把表的数据修改了并且提交了，此时事务B已经开启了事务，此时B读取的是A提交后的新数据。
   ...
   - 可重复读：repeatable read
     ...
     >
     > 幻象读：例如两个事务同一期间开启事务，事务A改了数据的名字，事务B改了数据的年龄，结果两个事务提交后发现，不光自己改的部分改了，另外一部分也改了(不知道另一部分是别人改的)
     >
     > 例如：事务A和事务B都同时begin开始了事务，事务A把表的数据修改了并且提交了，此时事务B已经开启了事务，即使读取也只是读取A修改之前的数据。
...
- 第一范式： 数据库表的每一列都是不可分割的原子数据项，而不能是集合，数组，记录等组合的数据项。简单来说要求数据库中的表示二维表，每个数据元素不可再分。
  ...
  例如： 在国内的话通常理解都是姓名是一个不可再拆分的单位，这时候就符合第一范式；但是在国外的话，国外人的姓名很长还要分为FIRST NAME和LAST NAME，这时候姓名这个字段就是还可以拆分为更小的单位的字段，就不符合第一范式了。

- 第二范式： 第二范式（2NF）要求数据库表中的每个实例或记录必须可以被唯一地区分，所有属性依赖于主属性。即选取一个能区分每个实体的属性或属性组，作为实体的唯一标识，每个属性都能被主属性筛选。其实简单理解要设置一个区分各个记录的主键就好了。# 意思就是设置主键
...
* **表常用存储引擎特点** 
  ...
  **InnoDB**（默认）		
	...
	# 行级锁：例如在开启的事务中，修改了表里的第一条数据，未提交的情况下这条数据会被锁定，其他进程无法对其写操作
	2. 支持外键、事务、事务回滚
  3. 表字段和索引同存储在一个文件中
  		 1. 表名.frm ：表结构
	 		 2. 表名.ibd : 表记录及索引文件
...
```
 ...
 1. 支持表级锁,在锁定期间，其它进程无法对该表进行写操作。如果你是写锁，则其它进程则读也不允许
  # 不支持外键、事务  # 对一个包含外键的InnoDB表转为MYISAM会失败
  # 表级锁：例如在开启的事务中，修改了表里的第一条数据，未提交的情况下这个表都会被锁定，其他进程无法对其写操作
  2.  表字段和索引分开存储   # 建的表会分成三个文件储存
        	 1. 表名.frm ：表结构
...
- 优先程度   数字 >  时间日期 > 字符串
- 同一级别   占用空间小的 > 占用空间多的     #  占用空间小的     好于   占用空间多的
...
- 少使用like %查询和正则查询，否则会全表扫描，效率不高

- 控制使用自定义函数

- 单条查询最后添加 LIMIT 1，停止全表扫描

- where子句中尽量不使用 != ,否则放弃索引全表扫描
...
# 例如：
create table book1 select * from books.book;  # 把books库里的book表复制到当前库的book1里
```
...
>
> mysqldump -u  root  -p  stu  >  ~/stu.sql     # 备份stu库 存在~下
...
3. 权限管理
   ...
   # 增加权限   # on 库A.*  给A库的所有表给权限  # with grant option 可以创建新用户可以把自己权限转给新用户
   grant 权限列表 on 库.表 to "用户名"@"%" identified by "密码" with grant option;
   
   # 删除权限
   revoke insert,update,select on 库.表 from 'user'@'%';
   # 例如 删除vip用户对stu.class的insert,update,select权限
   revoke insert,update,select on stu.class from 'vip'@'%';
4. 刷新权限
   flush privileges;
5. 删除用户
   drop user "用户名"@"%"
...
sql1 = "insert into class values(%s,%s,%s,%s,%s,%s)"    # 全部用%s就可以
...
db.close() 关闭连接
# 在pycharm运行的时候db.rollback()回滚，未必可以回复到原来状态，引擎是MyISAM的时候，运行一句提交一次，不需要commit提交，再回滚也无法恢复原来数据形态，因为MyISAM是没有事务的功能的。
```



* 文件存储
  * 存储文件路径   # 储存电影视频等等几百M以上的大文件建议使用该方法
    * 优点：节省数据库空间，提取方便
