...
* 部分命令细节说明
  ...
  * mkdir: -p选项可以创建层目录,创建多层目录：mkdir -p a/b/c   意思是创建a目录再在a里创建b,再在b里创建c
  * cp：如果拷贝的是一个目录需要使用 -r ，同时这个命令有另存为的作用，例如：cp file1.txt dir  意思是把file1.txt复制到dir文件夹里面，可以同时复制多个文件，也可以复制文件夹，要在文件夹前加 -r      # cp还有另存为功能：cp file1.txt file2.txt    # cp可以把别的目录的文件复制到当前目录，当前目录用“.”表示
  * mv:  移动一个文件，即使移动目录页不需要选项，有重命名的作用。 # mv还有重命名功能:mv day01 day02    如果day02存在则把day01移动到day02内，如果不存在则把day01重命名为day02 
...
| 06   | file  文件名  | 查看文件类型   |
| 07   | wc  文件名  | 查看文件行数，单词数，字符数等信息，行数显示的是换行符数量 |
| 08 | diff  文件1 文件2 | 对比两个文件差异 |

* 部分命令细节说明
  * head，tail ：选项-n，n表示一个数字，即可指定查看前n行或者后n行，不加选项默认查看10行。head -5  查看前5行        tail -5 查看后5行
...
* 部分命令细节说明
  * zip： 用于常与windows交互的情况，-r选项可以压缩目录
    ...
    * > zip    test.zip   filelist    # 把filelist文件压缩，压缩后压缩包名字test.zip 
      >
      > zip	-r	dir.zip	dir    #  把dir文件夹压缩 ，压缩后压缩包名字dir.zip
  ...
  * tar：-cjf 用于压缩bz2格式文件，-czf用于压缩gz格式文件，-xvf用于解压文件,兼容了gzip和bzip2命令的功能。
    ...
    * > tar -czf   file.tar.gz   file1  file2   # 把 file1和 file2 压缩成 file.tar.gz文件
  
    * > tar -xvf file.tar.gz   # 解压
...
* 部分命令细节说明
  ...
  * `chmod` 在设置权限时，可以字母也可以使用三个数字分别对应 **拥有者** u／ **组g** 和 **其他o** 用户的权限
  
    ls  -l   可以看到设置的权限   # r:可读   w:可写  x:可执行
  
     	-rwx-rw-rw-    # 第一个rwx代表的是 u的权限，第二个rw代表的是g的权限，第三个rw代表的是o的权限
    ...
    ![](C:\Users\Administrator\Desktop\学习笔记\第二阶段\Linux\img\chmod1.png)
...
* 部分命令细节说明
  ...
  * df:  -h选项以M或者G为单位显示，-T显示文件系统类型      ext4文件类型的为磁盘
  
    例如：df   -Th
  
  * which：命令也是一个程序，实际就是显示程序所在位置
  
    例如： which ls   显示ls命令在哪个位置存放
  
* 输出重定向

  | 重定向符  | 含义                                                     | 实例                                                         |
  | --------- | -------------------------------------------------------- | ------------------------------------------------------------ |
  | >   file  | 将file文件重定向为输出源，新建模式（清空文件内原内容）   | echo "hello world"   > out.txt，将执行结果，写到out.txt文件中，若有同名文件将被删除 |
  | >>   file | 将file文件重定向为输出源，追加模式（不清空文件内原内容） | ls   /usr   >> Lsoutput.txt，将ls   /usr的执行结果，追加到Lsoutput.txt文件已有内容后 |

  ​								echo "hello world"   > >out.txt，将执行结果，追加到out.txt文件已有的内容后
...
2. 对一些配置文件的修改，需要一定的权限，这时vi编辑器是最佳选择（自带管理员权限）。
...
* 工作模式
  ...
  2. **底行模式** —— 执行 **保存**、**退出** 等操作 
     ...
     - **末行模式** 是 `vi` 的 **出口**
     - 异常退出会生成一个隐藏文件，下次打开编辑需要删除该隐藏文件，ls   -a可以看隐藏文件
...
|    命令    | 功能                        |
| :---------:| --------------------------- |
|     yy     | 复制一行，可以 nyy 复制多行    # 例如 3yy   复制三行      快捷键：ctrl+shift+c |
|     dd     | 删除光标所在行，可以 ndd 复制多行  # 剪切 |
|      p     | 粘贴    快捷键：ctrl+shift+v   |
...
* passwd ： 设置密码，设置之后才能切换新用户登录
  ...
  sudo passwd levi       # 给levi用户设置密码

* 设置密码后为新用户添加sudo权限,打开sudoers文件增加如下内容，然后 :w! 强制保存 :q 退出

  ```
  sudo vi /etc/sudoers     # 使用vi打开etc目录下的sudoers文件
  找到  root	 ALL=(ALL:ALL) ALL 后在下面一行添加需要添加管理员权限的用户
  例如：levi		ALL=(ALL:ALL) ALL    #给levi用户添加管理员权限
  下图是给abby用户添加管理员权限
  ```
  ...
  注意：1. 新创建的用户和密码信息存储在 /etc/passwd文件中
       ...
       把sh解释器改成bash解释器之后，注销之后，打开终端 文件夹就可以显示各种颜色了
  ```

  ![](./img/user.png)

* userdel:  一般使用-r 彻底删除，如果删除失败说明刚刚使用了该用户，需要重启再删除。或者执行下面命令杀死正在运行的进程2314。那个-9可以不写
...
Linux下安装的软件包是 deb格式软件包。由于当时Linux系统中软件包存在复杂的依赖关系。因而，通常使用网络安装。,乌班图是使用apt操作升级，但是其他版本linux不一定使用apt
...
* 安装启动
  ...
  - 查看ssh服务状态 ： ps -e|grep ssh
    ...
    有sshd就说明安装了ssh服务，说明该电脑可以做服务器被远程访问
...
复制终端当前打开的文件夹下的文件到远程服务器：

scp  文件名  用户名@ip：远程服务器保存的绝对路径



复制远程服务器的文件到本地电脑：

scp  用户名@ip：远程服务器绝对路径   本地绝对路径    # 本地路径可以用用“.”代替，表示当前目录下
...
2. scp拷贝
   ...
   # 注意：`:` 后面的路径写绝对路径  # 在本地的demo.py所在文件夹下运行
   # 把本地文件demo.py拷贝到levi@192.168.100.5的/home/tarena文件夹下
   scp  demo.py levi@192.168.100.5:/home/tarena
   
   # 把远程主目录下demo.py文件 复制到本地当前目录下
   scp  levi@192.168.100.5:/home/tarena/demo.py  .    # 注意后面有个“.”
   exit
   # 加上 -r 选项可以传送文件夹
   scp -r demo levi@192.168.100.5:/home/tarena/
   
   # 把远程的/root/请不要删除/day02/2.txt文件拷贝到当前目录“.”下
   scp root@121.36.43.159:/root/请不要删除/day02/2.txt .     # 注意后面有个“.”
...
* ssh秘钥
  ...
  * 使用方法
    ...
    ```
    1. 在个人计算机中生产秘钥对 ： ssh-keygen  执行以后会在主目录下生成一个.ssh的隐藏文件夹,其中包含私钥文件id_rsa和公钥文件id_rsa.pub。
    2. 在服务器主机上创建文件~/.ssh/authorized_keys，将信任的计算机的id_rsa.pub文件内容追加到服务器authorized_keys文件中，并修改其权限为600。
    ```

    ![](C:\Users\Administrator\Desktop\学习笔记\第二阶段\Linux\img\ssh3.png)
