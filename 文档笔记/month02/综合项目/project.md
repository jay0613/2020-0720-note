...
* 配置命令： git config --global [选项]      
  ...
  ```
  # 例如配置名字  git config --global user.name Bill
  # 例如配置邮箱  git config --global user.email 240110075@qq.com
  # 查看已配置内容  cat ~/.gitconfig
  ```
...
* 初始化仓库
  ...
  ```shell
  git  init 
  意义：将某个项目目录变为git操作目录，生成git本地仓库。即该项目目录可以使用git管理  # cd进入这个文件夹之后再使用这个命令,就会把这个文件夹变成本地仓库
  ```
...
* 取消文件暂存记录
  ...
  ```shell
  git rm --cached [file] 
  # 例如 git rm --cached a      # 取消a文件暂存
  ```

* 设置忽略文件

  在GIT项目中可以在项目根目录添加**.gitignore**文件的方式，规定相应的忽略规则，用来管理当前项目中的文件的忽略行为。.gitignore 文件是可以提交到公有仓库中，这就为该项目下的所有开发者都共享一套定义好的忽略规则。在.gitignore 文件中，遵循相应的语法，在每一行指定一个忽略规则。”# 想要文件被忽略就写到.gitignore内就可以,注意写到这里面的文件若被删除无法恢复

	```shell
	.gitignore忽略规则简单说明
	
	file            表示忽略file文件  # 直接写对应的文件名到.gitignore里面
	*.a             表示忽略所有 .a 结尾的文件
	!lib.a          表示但lib.a除外
...
e.g.  将暂存区所有记录同步到仓库区  # 'add files'只是添加的说明，写啥都行
git commit  -m 'add files'

e.g.  将暂存区的index.html文件同步到仓库区
git commit index.html -m 'add files'
```
...
* 查看commit 日志记录
  ...
  ```shell
  git log            # 查看所有日志，包括commit_id日志版本号    按Q可以关闭
  git log --pretty=oneline   # 只显示日志版本号和备注信息
  ```
...
* 退回到指定的commit_id节点
  ...
  # adbd297 为版本号前7位
  git reset --hard adbd297    # 直接退回adbd297这个版本
  ```
...
* 创建标签
  ...
  * 标签: 在项目的重要commit位置添加快照，保存当时的工作状态，一般用于版本的迭代。
    ...
    e.g. 在adbd297版本处添加标签v0.9
    git tag v0.9 adbd297 -m '版本0.9'
...
* 保存工作区内容
  ...
  # 将当前工作区未提交的修改封存备注为“第一方案”，让工作区回到修改前的状态
  git stash save "第一方案"
  ```
...
* 合并分支
  ...
  ```  
  git merge [branch]
  git merge Bill   # 在master分支使用该代码  表示合并Bill分支
  ```
...
* 删除分支
  ...
  ```
   ...
   git branch -D [branch]  删除没有被合并的分支(强删)
  ```
...
- github仓库相对本地主机就是一个远程仓库通过remote连接
  ...
  ```
  git push -u origin Bill     # 上传Bill分支
  git push -u origin :Bill    # 删除Bill分支   # 注意别删除主分支
  ```
  ...
  - 使用SSH连接
    ...
    ```
  3. 将公钥内容添加进去，并且起一个标题名字，点击添加(注意添加在网页密匙时，下面的√打上)
